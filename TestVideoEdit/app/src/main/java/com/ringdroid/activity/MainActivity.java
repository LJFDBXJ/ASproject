...
public class MainActivity extends AppCompatActivity implements MarkerView.MarkerListener,
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mFilename = Environment.getExternalStorageDirectory().getAbsolutePath() +"/HMSDK/video/1531383835814.mp4";
        mKeyDown = false;
...
//        videoEncode.init(mFilename);
    ...
    private void loadGui() {
        ...
        videoView.postDelayed(mPlayerPrepare,100);

    }
    private Runnable mPlayerPrepare = new Runnable() {
        public void run() {
            if(player != null && player.getDuration() > 0 && player.getVideoFormat() != null){
                Thread thread = new Thread(){
                    ...
                    public void run() {
                        ...
                        while (mWaveformView.getImageWidth() == 0 && mWaveformView.getImageHeight() == 0){
                            try {
                                Thread.sleep(100);
                        ...
                        if(d < mWaveformView.getCutTime()*1000){
                            Bitmap bitmap = media.getFrameAtTime(d);
                            Bitmap b = Bitmap.createBitmap(mWaveformView.getImageWidth(),mWaveformView.getImageHeight(), Bitmap.Config.RGB_565);
                            Canvas canvas = new Canvas(b);
                            canvas.drawBitmap(bitmap,new Rect(0,0,bitmap.getWidth(),bitmap.getHeight()),new RectF(0,0,b.getWidth(),b.getHeight()),null);
                            bitmaps.add(b);
                            mWaveformView.invalidate();
                        }else{
                            int i;
                            for (i = 0;i<d;i+= mWaveformView.getCutTime()*1000){
                                Bitmap bitmap = media.getFrameAtTime(i*1000);
                                Bitmap b = Bitmap.createBitmap(mWaveformView.getImageWidth(),mWaveformView.getImageHeight(), Bitmap.Config.RGB_565);
                                Canvas canvas = new Canvas(b);
                                canvas.drawBitmap(bitmap,new Rect(0,0,bitmap.getWidth(),bitmap.getHeight()),new RectF(0,0,b.getWidth(),b.getHeight()),null);
                                bitmaps.add(b);
...
//                        videoImage.setImageBitmap(bitmap);
                ...
                playerView = new TextureView(MainActivity.this);
                ...
                cutView = new CutView(MainActivity.this);
