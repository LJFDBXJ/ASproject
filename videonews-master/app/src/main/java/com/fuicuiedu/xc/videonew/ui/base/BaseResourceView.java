package com.fuicuiedu.xc.videonew.ui.base;
...
import com.fuicuiedu.xc.videonew.bmobapi.NewsApi;
 ...
 */
public abstract class BaseResourceView<T, ItemView extends BaseItemView<T>>extends FrameLayout
        implements SwipeRefreshLayout.OnRefreshListener, MugenCallbacks {
    ...
    public BaseResourceView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        BaseResourceView_initView();
    }


    /**——————继承baseitemvie要实现的抽象方法初始化视图------*/
    private void BaseResourceView_initView() {
        /**NewsApi初始化*/
        newsApi = BmobClient.getInstance().getNewsApi();
    ...
    public void onRefresh() {
        ...
        Call<QueryResult<T>> call = queryData(getLimit(), 0);
        /**-----------      返回null，说明查询条件不足        --------------*/
        if (call == null) {
        ...
        call.enqueue(new Callback<QueryResult<T>>() {
            @Override
            public void onResponse(Call<QueryResult<T>> call, Response<QueryResult<T>> response) {
                refreshLayout.setRefreshing(false);
                /**从call的异步执行——拿到响应中的数据集合——并用对象接收*/
                List<T> datas = response.body().getResults();
                /**改变跳过数目（skip）————且datas.size就等于limit的值*/
                skip = datas.size();
                /**判断是否获取完服务器的数据——如果获取到的数据的个数小于limit的值，代表没数据了*/
                isloadAll =datas.size() < getLimit();
                /**将数据添加到adapter——调用刷新的方法，清空旧数据，并且更新了适配器*/
                adapter.clear();
                /**将传进来的数据强转成带泛型的集合*/
                adapter.addData((ArrayList<T>) datas);
            }

            /**请求失败的时候，让刷新停止————吐司错误的原因*/
            @Override
            public void onFailure(Call<QueryResult<T>> call, Throwable t) {
                refreshLayout.setRefreshing(false);
    ...
    * 加载更多的方法——onLoadMore*/
    ...
    public void onLoadMore() {
        ...
        Call<QueryResult<T>> call = queryData(getLimit(), skip);
        /**返回null，说明查询条件不足*/
        if (call == null) {
    ...
    /**根据布尔值loadAll——判断是否还有数据-----------------请求的数据数量小于限制数量*/
    @Override
...
//--------------------------------------------------------------------------------------------------
    /**抽像方法——从服务器查询数据(就是构建一个请求)*/
    protected abstract Call<QueryResult<T>> queryData(int limit, int skip);
...
//-------------------------------------------------------------------------------------------------
    ...
    protected class ModelAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
        ...
        public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
            ...
            /**将itemView视图填充到布局中————因为itemview是继承framelayout，所以有setlayoutparams方法
