...
//       views.setOnClickFillInIntent(this);
    ...
    public void onDisabled(Context context) {
        ...
        Toast.makeText(MyContext.getContext(),"onDisabled(Context context)最后一个Widget的实例被删除",Toast.LENGTH_LONG).show();
    }
    @Override
    public void onDeleted(Context context, int[] appWidgetIds) {
        super.onDeleted(context, appWidgetIds);
        Toast.makeText(MyContext.getContext(),"onDeleted(Context context, int[] appWidgetIds)当Widget被删，此方法被调用",Toast.LENGTH_LONG).show();
    }

    @Override
    public void onReceive(Context context, Intent intent) {
        super.onReceive(context, intent);
        Toast.makeText(MyContext.getContext(),"onReceive(Context context, Intent intent)接收到任意广播时触发，比上述方法优先调用",Toast.LENGTH_LONG).show();
